...
import { getFund, type PublicFundDetailView, type PublicFundView } from "../api";
  ...
  return (
    <div className="card fund-card">
      <div className="fund-card-top">
        <div className="fund-card-identity">
          ...
          <div className="fund-card-meta">
            ...
            <span className="fund-card-fee">FEE {(fund.feePct * 100).toFixed(2)}</span>
          </div>
        </div>

        <div className="fund-card-sparkline">
          <Sparkline prices={fund.sparkline} width={140} height={40} animate />
        </div>

        <div className="fund-card-values">
          <div className="fund-card-value-block">
            <span className="fund-card-nav-value">{fmtCoin(fund.nav)}</span>
            <span className="fund-card-value-label">Market Value</span>
          </div>
          <div className="fund-card-value-block">
            <span className={`fund-card-delta-value ${last7.cls}`}>{last7.text.split(" ")[0]}</span>
            <span className="fund-card-value-label">Last 7 Days</span>
          </div>
          <div className="fund-card-value-block">
            <span className={`fund-card-delta-value ${allTime.cls}`}>{allTime.text.split(" ")[0]}</span>
      ...
      <div className="fund-card-bottom">
        ...
        >
          <ArrowTrendingUpIcon className="fund-card-buy-icon" /> Buy Now
        </button>
      </div>

      <button type="button" className="fund-card-details-toggle" onClick={toggleExpanded}>
        Details
        {expanded ? (
      ...
      <div className={expanded ? "fund-card-details expanded" : "fund-card-details"}>
        <div>
              ...
              <tbody>
                {detail.constituents.map((c) => (
                  <tr key={`${c.playerName}::${c.server}`}>
                    <td>{c.playerName}</td>
                    <td>{c.stockCount}</td>
                    <td>{(c.percentOfFund * 100).toFixed(2)}%</td>
                    <td>{c.price !== null ? fmtCoin(c.price) : "–"}</td>
                  </tr>
                ))}
                {detail.constituents.length === 0 && (
                  <tr>
                    <td colSpan={4} className="subtitle">
                      This fund has no constituents right now.
                    </td>
                  </tr>
                )}
              </tbody>
